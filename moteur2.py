# ==================================================
# ASSISTANT IT PRO - Version 3.0
# Copyright © 2026 Assistant IT Pro
...
# CONFIGURATION
# ==================================================
DB = "assistant_it_pro.db"   # <--- AJOUTER ICI
st.set_page_config(
...
#changer#
IBAN = os.getenv("IBAN", "BE80 9733 8252 3877")
BIC = os.getenv("BIC", "ARSPBE22XXX")
CONTACT_EMAIL = os.getenv("CONTACT_EMAIL", "tech.contactinformatique@proton.me")
...
class Database:
    ...
    def _init_db(self):
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS utilisateurs (
                ...
                recherches_jour INTEGER DEFAULT 0,
                date_recherche TEXT,
    ...
    def create_user(self, email, password_hash):
        ...
        try:
            cur.execute("""
                ...
                (email, password, plan, premium, recherches, date_inscription, recherches_jour, date_recherche)
                VALUES (?, ?, 'gratuit', 0, 0, ?, 0, ?)
            """, (email, password_hash, datetime.now().isoformat(), date.today().isoformat()))
            conn.commit()
            conn.close()
            return True
    ...
    def update_user_plan(self, email, plan, premium=1, expire_le=None):
        conn = self._get_conn()
        cur = conn.cursor()
        cur.execute("""
            UPDATE utilisateurs 
            SET plan = ?, premium = ?, abonnement_expire_le = ?
            WHERE email = ?
        """, (plan, premium, expire_le, email))
        conn.commit()
        conn.close()

    def update_daily_search_count(self, email, count, today):
        conn = self._get_conn()
        cur = conn.cursor()
        cur.execute("""
            UPDATE utilisateurs 
            SET recherches_jour = ?, date_recherche = ?
            WHERE email = ?
        """, (count, today.isoformat(), email))
        conn.commit()
        conn.close()

    def get_daily_search_count(self, email):
        conn = self._get_conn()
        cur = conn.cursor()
        cur.execute("SELECT recherches_jour, date_recherche FROM utilisateurs WHERE email = ?", (email,))
        row = cur.fetchone()
        conn.close()
        if row:
            return row[0], row[1]
    ...
    def check_and_update_expired(self, email):
        conn = self._get_conn()
        cur = conn.cursor()
        cur.execute("SELECT abonnement_expire_le, plan, premium FROM utilisateurs WHERE email = ?", (email,))
        row = cur.fetchone()
        if row and row[0]:
            try:
                expire = datetime.fromisoformat(row[0])
                if datetime.now() > expire and row[2] == 1:
                    cur.execute("""
                        UPDATE utilisateurs 
    ...
    def add_password_reset_token(self, email, token, expire):
        conn = self._get_conn()
        cur = conn.cursor()
        cur.execute("INSERT INTO password_resets (email, token, expire_le) VALUES (?, ?, ?)",
                    (email, token, expire.isoformat()))
        conn.commit()
        conn.close()
    ...
    def get_password_reset_token(self, token):
        conn = self._get_conn()
        cur = conn.cursor()
        cur.execute("SELECT email, expire_le FROM password_resets WHERE token = ?", (token,))
        row = cur.fetchone()
        conn.close()
        return row
    ...
    # ------------------- PANNES -------------------
    def get_all_pannes(self):
        conn = self._get_conn()
        df = pd.read_sql_query("SELECT * FROM pannes", conn)
        conn.close()
        return df

    def insert_initial_data(self, data):
        conn = self._get_conn()
        cur = conn.cursor()
...
class RechercheIT:
    ...
    def charger(self):
        if self.df is None:
            self.df = self.db.get_all_pannes()
...
def inscription(db: Database, email: str, password: str):
...
def connexion_utilisateur(db: Database, email: str, password: str):
    user = db.get_user_by_email(email)
    if user and verify_password(password, user[2]):
        db.check_and_update_expired(email)
        user = db.get_user_by_email(email)
        logging.info(f"Connexion de {email}")
        return user
    return None
    ...
    if st.session_state.montant_virement > 0:
        ...
        with col1:
            ...
            <div style='background: #1a1a2e; padding: 20px; border-radius: 10px; border: 1px solid #2a2a4a;'>
                ...
                <p><strong style='color:#aaa;'>Titulaire :</strong> <span style='color:white;'>Assistant IT Pro</span></p>
                <p><strong style='color:#aaa;'>IBAN :</strong> <span style='color:white;'>{IBAN}</span></p>
                <p><strong style='color:#aaa;'>BIC :</strong> <span style='color:white;'>{BIC}</span></p>
        ...
        with col2:
            ...
            <div style='background: #1a1a2e; padding: 20px; border-radius: 10px; border: 1px solid #2ecc71;'>
                ...
                <p><strong style='color:#aaa;'>Email :</strong> <span style='color:white;'>{CONTACT_EMAIL}</span></p>
...
def page_licence():
    ...
    - Tous les droits de propriété intellectuelle sur le logiciel **Assistant IT Pro** appartiennent à **Assistant IT Pro**.
    ...
    - Conformément au RGPD, vous pouvez demander la suppression de vos données à tout moment en contactant {CONTACT_EMAIL}.
    ...
    ### 🛡️ Sécurité
    - Les mots de passe sont hachés avec **bcrypt** (sel unique).
...
# ==================================================
# DONNÉES INITIALES (LISTE COMPLÈTE)
# ==================================================
...
DONNEES_INITIALES = [
    ...
    ("Windows Update tourne en boucle", "Windows Update se bloque", "Mise à jour bloquée",
     ...
     "update,windows"),
(r"C:\ plein", "Le disque C: est saturé", "Fichiers temporaires ou applications", "1- Nettoyer le disque\n2- Vider la corbeille\n3- Désinstaller des applications", "Quelle taille fait-il ?", "Windows", 2, "disque,plein,windows"),
(r"D:\ non reconnu", "Le disque D: n'apparaît plus", "Problème de partition ou de câble", "1- Vérifier le câble\n2- Utiliser Gestion des disques\n3- Tester le disque", "Le disque fait-il du bruit ?", "Windows", 4, "disque,partition,windows"),
...
def main():
    db = Database(DB)
    db.insert_initial_data(DONNEES_INITIALES)

    if st.session_state.moteur is None:
        st.session_state.moteur = RechercheIT(db)

    # ==================================================
    # SIDEBAR (identique à la version 3.0)
    # ==================================================
    ...
    with st.sidebar:
        ...
        if st.session_state.user:
            ...
            user = db.get_user_by_email(st.session_state.user)
            if user:
            ...
            count, last_date = db.get_daily_search_count(st.session_state.user)
            today = date.today()
            if last_date and last_date == today.isoformat():
                restant = max(0, 3 - count) if not st.session_state.premium else "∞"
            else:
                restant = 3 if not st.session_state.premium else "∞"
                count = 0
                db.update_daily_search_count(st.session_state.user, 0, today)

            if not st.session_state.premium:
                st.markdown(f'<p style="color:#FFFFFF;">🔍 {restant} recherches restantes</p>', unsafe_allow_html=True)
                st.progress(count / 3 if count < 3 else 1.0)
            else:
                st.markdown('<p style="color:#00d4ff;">🔍 Recherches illimitées</p>', unsafe_allow_html=True)
            ...
            with tab1:
                ...
                if st.button("Se connecter", use_container_width=True):
                    user = connexion_utilisateur(db, email, password)
                    if user:
            ...
            with tab2:
                ...
                if st.button("Créer un compte", use_container_width=True):
                    success, msg = inscription(db, email, password)
                    if success:
                        st.success(msg)
                    else:
                        st.error(msg)
    ...
    st.markdown(
        ...
        unsafe_allow_html=True)
    st.markdown(
        '<p style="color:#aaa; text-align:center; font-size:18px;">Par Assistant IT Pro - <span style="color:#FFD700;">1000+ diagnostics</span></p>',
        unsafe_allow_html=True)
    st.markdown("---")
    ...
    with col2:
        if st.button("🔍 Rechercher", type="primary", use_container_width=True):
            if not st.session_state.user:
                ...
                db.check_and_update_expired(st.session_state.user)
                user = db.get_user_by_email(st.session_state.user)
                if user:
                ...
                if question.strip():
                    with st.spinner("Recherche en cours..."):
                        if not st.session_state.premium:
                            count += 1
                            db.update_daily_search_count(st.session_state.user, count, date.today())
    ...
    st.markdown("---")
    st.markdown(
        f'© 2026 <strong style="color:#FFD700;">Assistant IT Pro</strong> - Tous droits réservés - Contact : {CONTACT_EMAIL}',
        unsafe_allow_html=True)
